...
function logicalOr(get) {
    var left = { type: 'logicalOr', right: logicalAnd(get) };
    for (;;) {
...
function logicalAnd(get) {
    var left = { type: 'logicalAnd', right: equality(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'LOGICAL_AND': {
...
function equality(get) {
    var left = { type: 'equality', right: comparison(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'EQUAL': {
...
function comparison(get) {
    var left = { type: 'comparison', right: membership(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'LESS_THAN': {
...
function membership(get) {
    var left = { type: 'membership', right: expr(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'IN': {
