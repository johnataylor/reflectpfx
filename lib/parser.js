...
function prim(get) {
    ...
    switch (currentToken.type) {
        case 'NUMBER': {
            ...
            getToken();
            return postFixPercent({ type: 'primary', match: 'number', value: v });
        }
        case 'TEXT_LITERAL': {
            var v = currentToken.value;
            getToken();
        ...
        case 'NAME': {
            ...
            if (currentToken.type === 'LP') {
                ...
                if (currentToken.type === 'RP') {
                    getToken();
                    return postFixPercent({ type: 'primary', match: 'function', name: name });
                }
                else {
                    ...
                    getToken();
                    return postFixPercent({ type: 'primary', match: 'function', name: name, args: arguments });
                }
            }
            // variable
            else {
                return postFixPercent({ type: 'primary', match: 'variable', name: name });
            }
        }
        case 'AND': {
            getToken();
        ...
        case 'LP': {
            ...
            getToken();
            return postFixPercent({ type: 'primary', match: 'parenthesis', expression: e });
        }
        case 'SLP': {
            getToken();
...
function postFixPercent(primary) {
    if (currentToken.type === 'PERCENT') {
        getToken();
        return { type: 'primary', match: 'percent', primary: primary };
    }
    return primary;
}
