...
function args(get) {
    var left = { type: 'args', right: bexpr(get) };
    for (;;) {
...
function records(get) {
    ...
    var left = { type: 'record', name: name, right: bexpr(true) };
    for (;;) {
        switch (currentToken.type) {
            case 'COMMA': {
                getToken();
...
function bexpr(get) {
    var left = { type: 'bexpression', right: expr(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'LESS_THAN': {
...
function expr(get) {
    var left = { type: 'expression', right: term(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'PLUS': {
...
function term(get) {
    var left = { type: 'term', right: prim(get) };
    for (;;) {
        switch (currentToken.type) {
            case 'MUL': {
...
function prim(get) {
    ...
    switch (currentToken.type) {
        ...
        case 'NOT': {
            return { type: 'primary', match: 'not', child: bexpr(true) };
        }
        case 'BANG': {
            return { type: 'primary', match: 'bang', child: bexpr(true) };
        }
        case 'MINUS': {
