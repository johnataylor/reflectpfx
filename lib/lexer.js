...
function processStringInterpolation(s, token) {
    ...
    function literal(s) {
        if (s.length > 0) {
            ...
            // mock up a concat '&' operator
            tokens.push({ type: 'CONCAT', value: '&' });
        }
    }

    function expression(s) {
        // note recursive call to the tokenizer
        var nestedTokens = tokenize(s);
        nestedTokens.pop();

        // mock up a 'Text()' function call around the expression
    ...
    var tokens = [];
    tokens.push({ type: 'BEGIN_STRING_INTERPOLATION', value: '' });
    var indexStart = stringInterpolation.read(s, token.index + token.value.length, literal, expression);

    // remove the last concat '&' operator because we will have added to many
    if (tokens.length > 1) {
        tokens.pop();
    }
